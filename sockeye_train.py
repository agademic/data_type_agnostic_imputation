    ...
    try:
        ...
        for typ in types:
            ...
            for miss in missingness:
                print(miss)
                try:
                    command_prepr = f'python -m sockeye.train -d {dataset}/{typ}/{miss}/prepared_sockeye_data/ -vs {dataset}/{typ}/{miss}/dev.source -vt {dataset}/{typ}/{miss}/dev.target -vsf {dataset}/{typ}/{miss}/dev.source_factors --device-ids -1 --optimized-metric accuracy --metrics accuracy --batch-type sentence --max-num-checkpoint-not-improved 3 -o {dataset}/{typ}/{miss}//sockeye_model_xxl'
                    print(command_prepr)
