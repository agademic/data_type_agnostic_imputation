...
for dataset in datasets:
    try:
        command_prepr = 'python -m sockeye.train -d {name}/prepared_sockeye_data/ -vs {name}/dev.source -vt {name}/dev.target -vsf {name}/dev.source_factors --num-embed 32 --source-factors-num-embed 16 --transformer-model-size 32 --transformer-feed-forward-num-hidden 16 --num-layers 4 --metrics perplexity accuracy --use-cpu --batch-type sentence --max-num-checkpoint-not-improved 3 --batch-size 4 --overwrite-output -o {name}/sockeye_model_large'.format(name=dataset)
        print(command_prepr)
